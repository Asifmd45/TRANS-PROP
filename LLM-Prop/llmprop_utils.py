...
def compressCheckpointsWithTar(filename):
    ...
    with tarfile.open(f"{filename_for_tar}.tar.gz", "w:gz") as tar:
        tar.add(filename)
...
def decompressTarCheckpoints(tar_filename):
    with tarfile.open(tar_filename) as tar:
        ...
        for member in tar.getmembers():
            ...
            if os.path.commonpath([extract_path, member_path]) != os.path.abspath(extract_path):
...
def get_cleaned_stopwords():
    ...
    stopword_files = glob.glob(os.path.join(base_dir, "stopwords", "en", "*.txt"))
...
def get_roc_score(predictions, targets):
    x = torch.tensor(targets).detach().cpu().numpy()
    y = torch.sigmoid(torch.tensor(predictions)).detach().cpu().numpy()
    if len(set(x.tolist())) < 2:
        return 0.0
    return float(roc_auc_score(x, y))